...
if [ ! "$USE_LOCAL_SOURCE" = "true" ] ; then
  # Downloading glib source bundle file. The '-c' option allows the download to resume.
  echo "Downloading glib source bundle from $DOWNLOAD_URL"
  wget -c $DOWNLOAD_URL
else
  echo "Using local glib source bundle $MAIN_SRC_DIR/source/overlay/$ARCHIVE_FILE"
fi
